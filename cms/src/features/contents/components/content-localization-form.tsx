...
export function ContentLocalizationForm({
  ...
  const showsLocalizationCover =
    content.summary.type === "STORY";
  const showsLocaleMediaMetadata = content.summary.type !== "LULLABY";
    ...
    try {
      const savedLocalization = await toastMutation(
        saveLocalization.mutateAsync({
          mode,
          values,
          contentType: content.summary.type,
        }),
        {
    ...
    >
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        {showsLocalizationCover ? (
          ...
          <div
            className="space-y-5 rounded-2xl border border-border/70 bg-muted/10 p-4 md:col-span-2"
            data-testid="content-localization-metadata-row"
          >
            {showsLocaleMediaMetadata ? (
              <>
                <div className="space-y-2">
                  <label className="text-sm font-medium text-foreground" htmlFor="durationMinutes">{copy.durationMinutes}</label>
                  <Input id="durationMinutes" inputMode="numeric" placeholder={copy.optional} type="number" {...form.register("durationMinutes", { setValueAs: (value) => value === "" || value == null ? null : Number(value) })} disabled={saveLocalization.isPending} />
                  <FieldError error={form.formState.errors.durationMinutes} />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium text-foreground">{copy.processingStatus}</label>
                  <Controller control={form.control} name="processingStatus" render={({ field }) => <Select disabled={saveLocalization.isPending} value={field.value} onValueChange={field.onChange}><SelectTrigger aria-label={copy.processingStatus} className="w-full"><SelectValue placeholder={copy.selectProcessingStatus} /></SelectTrigger><SelectContent>{processingStatusOptions.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectContent></Select>} />
                </div>
              </>
            ) : null}
            <div className="space-y-2">
              <label className="text-sm font-medium text-foreground">{copy.status}</label>
              <Controller control={form.control} name="status" render={({ field }) => <Select disabled={saveLocalization.isPending} value={field.value} onValueChange={field.onChange}><SelectTrigger aria-invalid={Boolean(form.formState.errors.status)} aria-label={copy.status} className="w-full"><SelectValue placeholder={copy.selectStatus} /></SelectTrigger><SelectContent>{localizationStatusOptions.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectContent></Select>} />
              <FieldError error={form.formState.errors.status} />
            </div>
            {status === "PUBLISHED" ? (
              <div className="space-y-2">
                <label className="text-sm font-medium text-foreground" htmlFor="publishedAt">{copy.publishedAt}</label>
                <Input id="publishedAt" type="datetime-local" {...form.register("publishedAt")} disabled={saveLocalization.isPending} />
                <p className="text-sm text-muted-foreground">{copy.publishedHelp}</p>
                <FieldError error={form.formState.errors.publishedAt} />
              </div>
            ) : null}
          </div>
