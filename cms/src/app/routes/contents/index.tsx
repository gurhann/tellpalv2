...
const readinesses: Array<ContentRegistryReadiness | "ALL"> = [
  "ALL",
  "ACTION_REQUIRED",
  "READY_TO_PUBLISH",
  "PUBLISHED",
...
export function ContentsIndexRoute() {
  ...
  const stateLabel = (state: ContentRegistryReadiness | "ALL") =>
    state === "ALL"
      ? locale === "tr"
        ? "Tüm durumlar"
        : "All statuses"
  ...
  return (
    <>
      <ContentPageShell
        eyebrow={locale === "tr" ? "Editoryal çekirdek" : "Editorial core"}
        title={locale === "tr" ? "İçerikler" : "Contents"}
        description={
          locale === "tr"
            ? "Seçili dildeki yayın engellerini bulun ve ilgili editöre geçin."
            : "Find selected-locale blockers and open the relevant editor."
        }
        actions={
          <>
            <Button
        ...
        toolbar={
          <RegistryToolbar
              ...
              >
                <div className="relative min-w-[16rem] flex-1">
                  ...
                  <Input
                    ...
                    placeholder={
                      locale === "tr"
                        ? "Başlık, anahtar veya ID"
                        : "Title, key, or ID"
                    }
                    onChange={(event) => {
                      setSearch(event.target.value);
                      update({ q: event.target.value.trim() || null });
                    }}
                  />
                </div>
              </RegistryToolbarGroup>
              ...
              <>
                ...
                >
                  <div className="flex flex-wrap gap-2">
                    {["tr", "en", "de", "es", "pt"].map((value) => (
                      <Button
                        key={value}
                        type="button"
                        size="sm"
                        variant={language === value ? "secondary" : "outline"}
                ...
                <RegistryToolbarGroup label={locale === "tr" ? "Tür" : "Type"}>
                  <div className="flex flex-wrap gap-2">
                    {types.map((value) => (
                      <Button
                        key={value}
                ...
                <RegistryToolbarGroup
                  label={locale === "tr" ? "Durum" : "Readiness"}
                >
                  <div className="flex flex-wrap gap-2">
                    {readinesses.map((value) => (
                      <Button
                        key={value}
                        type="button"
                        size="sm"
                        variant={readiness === value ? "secondary" : "outline"}
            ...
            summaryTitle={
              locale === "tr"
                ? `${registry.registry?.totalItems ?? 0} sonuç · ${language.toUpperCase()}`
                : `${registry.registry?.totalItems ?? 0} results · ${language.toUpperCase()}`
            }
            summaryDescription={
              locale === "tr"
      ...
      >
        <ContentListTable
          ...
          onContentSelect={(item) =>
            navigate(`/contents/${item.contentId}?language=${language}`)
          }
        />
        <Pagination
          page={page}
