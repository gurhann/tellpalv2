...
- [x] [Review][Patch][medium] Create/update localization yanıtlarında narration processing durumu ve hatası dönmüyor [`be/src/main/java/com/tellpal/v2/content/web/admin/AdminContentLocalizationResponse.java:31-45`](../../be/src/main/java/com/tellpal/v2/content/web/admin/AdminContentLocalizationResponse.java#L31-L45) — mapper ve publication projection processing-aware narration alanlarını taşıyor; HTTP testi PENDING durumunu doğruluyor.
- [x] [Review][Patch][medium] Narration request/response ve `STORY_NARRATION` retry kuralları canonical admin dokümanında yok [`be/docs/admin-api-rules.md`](../../be/docs/admin-api-rules.md) — nested sözleşme, kind ayrımı, hata görünürlüğü ve retry/page-count kuralları dokümante edildi.
- [x] [Review][Patch][medium] `STORY_NARRATION` schedule/retry komutları `pageCount` boş bırakıldığında geçerli narration isteğini reddediyor [`be/src/main/java/com/tellpal/v2/asset/api/AssetProcessingCommands.java:55-61`](../../be/src/main/java/com/tellpal/v2/asset/api/AssetProcessingCommands.java#L55-L61) — narration için `pageCount` null/0 normalize ediliyor; pozitif değer reddediliyor.
- [x] [Review][Patch][medium] `STORY_NARRATION` işinde `audioSourceAssetId` null kalabiliyor [`be/src/main/java/com/tellpal/v2/asset/api/AssetProcessingCommands.java:55-63`](../../be/src/main/java/com/tellpal/v2/asset/api/AssetProcessingCommands.java#L55-L63) — command ve domain katmanında zorunlu audio source doğrulaması eklendi.
- [x] [Review][Patch][high] Aynı narration payload'ı ile yapılan ilgisiz localization güncellemesi processing job'ını gereksiz yere yeniden planlıyor veya `PROCESSING` işinde güncellemeyi başarısız kılıyor [`be/src/main/java/com/tellpal/v2/content/application/ContentManagementService.java:122-141`](../../be/src/main/java/com/tellpal/v2/content/application/ContentManagementService.java#L122-L141) — yalnızca kaynak asset'i veya süre değiştiğinde schedule/retry çalışıyor; aynı payload korunuyor.
- [x] [Review][Patch][medium] Veritabanı `STORY_NARRATION` kaydını localization/STORY hedefiyle sınırlamıyor [`be/src/main/resources/db/migration/V23__add_story_narrations.sql:14-26`](../../be/src/main/resources/db/migration/V23__add_story_narrations.sql#L14-L26) — V23 check constraint ve mevcut STORY localization trigger'ı ile scope/type garantilendi.
- [x] [Review][Patch][medium] Narration asset türü, bulunamayan asset ve rollback davranışı için entegrasyon testi eksik [`be/src/test/java/com/tellpal/v2/asset/AssetProcessingIntegrationTest.java:261-293`](../../be/src/test/java/com/tellpal/v2/asset/AssetProcessingIntegrationTest.java#L261-L293) — geçersiz asset türü/kaynak ve persist olmama senaryoları Testcontainers testiyle kapsandı.
- [x] [Review][Patch][medium] V23 migration backfill ve narration/processing DB constraint'leri migration testleriyle doğrulanmıyor [`be/src/main/resources/db/migration/V23__add_story_narrations.sql:1-52`](../../be/src/main/resources/db/migration/V23__add_story_narrations.sql#L1-L52) — legacy `DELIVERY` default/backfill, STORY dışı insert, duplicate ve localization cascade doğrulandı.
- [x] [Review][Patch][medium] Completed/failed narration güncellemesinin aynı kind-specific satırı `PENDING` durumuna aldığı doğrulanmıyor [`be/src/main/java/com/tellpal/v2/asset/application/AssetProcessingService.java:206-243`](../../be/src/main/java/com/tellpal/v2/asset/application/AssetProcessingService.java#L206-L243) — tamamlanmış narration kaynak değişiminde yeni asset ile `PENDING` state assertion'ı eklendi.
- [x] [Review][Patch][medium] Admin POST/PUT nested narration sözleşmesi ve GET projection'ı HTTP entegrasyon testiyle doğrulanmıyor [`be/src/main/java/com/tellpal/v2/content/web/admin/ContentAdminController.java:285-331`](../../be/src/main/java/com/tellpal/v2/content/web/admin/ContentAdminController.java#L285-L331) — POST/PUT response, processing state ve STORY dışı 400 testi eklendi.
- [x] [Review][Patch][medium] CMS narration alanlarının mevcut veriyi doldurma ve nested payload üretme davranışı test edilmiyor [`cms/src/features/contents/schema/content-localization-schema.ts:149-208`](../../cms/src/features/contents/schema/content-localization-schema.ts#L149-L208) — story form prefill, eşleşik audio/süre şeması ve mutation serialization test edildi.
- [x] [Review][Patch][medium] Çoklu dil narration izolasyonu test edilmiyor [`be/src/main/java/com/tellpal/v2/content/application/ContentAdminQueryMapper.java:38-42`](../../be/src/main/java/com/tellpal/v2/content/application/ContentAdminQueryMapper.java#L38-L42) — tr güncellemesinin en localization ve processing kaydını değiştirmediği entegrasyon testiyle doğrulandı.
