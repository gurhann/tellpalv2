...
def main() -> int:
    ...
    try:
        ...
        if plan.missing_body_sources and (
            not getattr(plan, "allow_missing_body", False)
            or getattr(plan, "publish", True)
        ):
...
def _add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--allow-missing-body",
        action="store_true",
        help="Stage bodyless MEDITATION localizations as DRAFT/PENDING (requires --no-publish)",
    )
