...
from lullaby_manifest import LullabyPlan


class LullabyImportRunReport:
    """Persists non-secret lullaby import state for manual diagnosis."""

    def __init__(self, plan: LullabyPlan, api_base_url: str):
    ...
    def record_asset(self, *, asset_id: int, media_key: str, source_name: str, kind: str, checksum_sha256: str, byte_size: int) -> None:
...
def run_storage_root() -> Path:
    ...
    if local_app_data:
        return Path(local_app_data) / "TellPal" / "lullaby-import-agent" / "runs"
    return Path.home() / ".local" / "share" / "TellPal" / "lullaby-import-agent" / "runs"
