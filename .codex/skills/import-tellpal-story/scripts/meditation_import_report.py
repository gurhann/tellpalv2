...
from meditation_manifest import MeditationPlan


class MeditationImportRunReport:
    """Persists non-secret meditation import state for manual diagnosis."""

    def __init__(self, plan: MeditationPlan, api_base_url: str):
...
def run_storage_root() -> Path:
    ...
    if local_app_data:
        return Path(local_app_data) / "TellPal" / "meditation-import-agent" / "runs"
    return Path.home() / ".local" / "share" / "TellPal" / "meditation-import-agent" / "runs"
